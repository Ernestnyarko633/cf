/** Pipepline Script for deploying Neobank Customer to Kubernetes Cluster
* @Author Peter Yefi
    ...
    if (env.BRANCH_NAME == 'main' || env.BRANCH_NAME == 'develop' ) {
       /* stage('SonarQube Analysis'){
            withSonarQubeEnv('Sonarqube'){
                withCredentials([string(credentialsId: 'neobank-sonarqube-analysis', variable: 'NEOBANK_SONARQUBE_ANALYSIS')]) {
                    sh "mvn clean package sonar:sonar \
                     -Dsonar.projectKey=neobank-gateway-service \
                     -Dsonar.host.url=${env.SONARQUBE_URL} \
                     -Dsonar.login=${NEOBANK_SONARQUBE_ANALYSIS} \
                     -Dsonar.java.jdkHome=/usr/java/openjdk-17"
